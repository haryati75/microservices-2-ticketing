...
describe('POST /api/users/signup', () => {
  ...
  it('should return 400 with password less than 4 characters', async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body.errors).toBeDefined();
    expect(response.body.errors[0].msg).toBe('Password must be between 4 and 20 characters');
  });
  ...
  it('should return 400 when email is missing', async () => {
    ...
    const emailError = response.body.errors.find(
      (err: any) => err.path === 'email' && err.value === undefined
    );
  ...
  it('should return 400 when password is missing', async () => {
    ...
    const passwordError = response.body.errors.find(
      (err: any) => err.path === 'password' && err.value === undefined
    );
  ...
  it('should return 400 when both email and password are missing', async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body.errors).toBeDefined();
    expect(response.body.errors).toHaveLength(4);  // Two errors for email, two for password
